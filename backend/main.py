...
class ScenarioInput(BaseModel):
    ...
    vegetation_cover: float = Field(..., ge=0, le=100, description="Vegetation Cover percentage (0-100)")
    renewable_energy_pct: float = Field(..., ge=0, le=100, description="Renewable Energy share (0-100)")

class SubscriptionRequest(BaseModel):
    email: EmailStr = Field(..., description="Subscriber email address")
    country: str = Field(..., min_length=2, max_length=100, description="Target country for alerts")
...
# =====================================================================
CITIES_DB = {
    "karachi": {"name": "Karachi", "lat": 24.8607, "lon": 67.0011, "country": "Pakistan"},
    "london": {"name": "London", "lat": 51.5074, "lon": -0.1278, "country": "United Kingdom"},
    "new york": {"name": "New York", "lat": 40.7128, "lon": -74.0060, "country": "United States"},
    "tokyo": {"name": "Tokyo", "lat": 35.6762, "lon": 139.6503, "country": "Japan"},
    "lahore": {"name": "Lahore", "lat": 31.5204, "lon": 74.3587, "country": "Pakistan"},
    "sydney": {"name": "Sydney", "lat": -33.8688, "lon": 151.2093, "country": "Australia"},
    "cairo": {"name": "Cairo", "lat": 30.0444, "lon": 31.2357, "country": "Egypt"},
    "sao paulo": {"name": "São Paulo", "lat": -23.5505, "lon": -46.6333, "country": "Brazil"},
    ...
    "dubai": {"name": "Dubai", "lat": 25.2048, "lon": 55.2708, "country": "United Arab Emirates"},
    ...
    "islamabad": {"name": "Islamabad", "lat": 33.6844, "lon": 73.0479, "country": "Pakistan"}
}
...
class MicroclimateMLEngine:
    ...
    def _initialize_and_train(self):
        try:
            if HAS_SKLEARN:
                # Generate synthetic microclimate training set
                np.random.seed(42)
                X = np.random.uniform(low=[0, 0, 0, -90, -180], high=[100, 100, 2000, 90, 180], size=(1000, 5))
                # Target formula: Higher urban density & lower vegetation -> higher temp anomaly
                y = (0.05 * X[:, 0]) - (0.04 * X[:, 1]) - (0.001 * X[:, 2]) + (0.02 * np.abs(X[:, 3])) + np.random.normal(0, 0.2, 1000)
                
                self.model = RandomForestRegressor(n_estimators=50, random_state=42)
                self.model.fit(X, y)
                self.is_trained = True
                logger.info("Microclimate Machine Learning Regressor trained successfully.")
            else:
                logger.warning("Scikit-Learn not found. Falling back to heuristic mathematical model.")
        except Exception as e:
            logger.error(f"Error training ML model: {e}")
            self.is_trained = False

    def predict(self, urban_density: float, vegetation_cover: float, elevation: float, lat: float, lon: float) -> float:
        if self.is_trained and self.model:
            features = np.array([[urban_density, vegetation_cover, elevation, lat, lon]])
            pred = float(self.model.predict(features)[0])
            return round(pred, 2)
        else:
            # Fallback heuristic calculation
            anomaly = (0.04 * urban_density) - (0.03 * vegetation_cover) - (0.0008 * elevation) + (0.01 * abs(lat))
            return round(anomaly, 2)

    def explain(self, urban_density: float, vegetation_cover: float, elevation: float, lat: float, lon: float) -> Dict[str, Any]:
        base_value = 1.25
        ...
        return {
            ...
            "feature_contributions": {
                "urban_density": {"value": urban_density, "impact": contrib_urban},
                "vegetation_cover": {"value": vegetation_cover, "impact": contrib_veg},
                "elevation_m": {"value": elevation, "impact": contrib_elevation},
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            result = {
                ...
                "humidity": data.get("hourly", {}).get("relativehumidity_2m", [60])[0],
                "rain_probability": data.get("hourly", {}).get("precipitation_probability", [15])[0],
                "is_mock": False
            }
            cache.set(cache_key, result, ttl_seconds=600)
            return result
    except Exception as e:
        logger.warning(f"External Weather API unavailable ({e}). Triggering Graceful Mock Fallback.")

    # Realistic Mock Fallback
    mock_data = {
        "temperature": round(22.0 + (abs(lat) * -0.2) + random.uniform(-3, 3), 1),
...
# =====================================================================
# 8. REST API ENDPOINTS (ALL 17 ENDPOINTS)
# =====================================================================
...
def get_weather_radar(lat: float = Query(24.8607), lon: float = Query(67.0011)):
...
def get_wildfires():
    ...
    # Simulated NASA FIRMS active fire hotspots
    fires = [
        {"id": "fire_01", "lat": -3.1190, "lon": -60.0217, "confidence": "High", "brightness_k": 345.2, "location": "Amazon Basin"},
        {"id": "fire_02", "lat": -31.9505, "lon": 115.8605, "confidence": "High", "brightness_k": 362.0, "location": "Western Australia"},
        {"id": "fire_03", "lat": 36.7783, "lon": -119.4179, "confidence": "Medium", "brightness_k": 320.8, "location": "California, USA"},
        {"id": "fire_04", "lat": 37.9838, "lon": 23.7275, "confidence": "High", "brightness_k": 338.4, "location": "Attica, Greece"},
        {"id": "fire_05", "lat": -14.2350, "lon": -51.9253, "confidence": "Low", "brightness_k": 310.1, "location": "Cerrado, Brazil"}
    ]
    response = {"count": len(fires), "fires": fires, "timestamp": datetime.utcnow().isoformat()}
...
def get_marine_heatwaves(lat: float = Query(0.0), lon: float = Query(0.0)):
    ...
    risk = "Extreme Coral Bleaching Risk" if anomaly > 2.5 else ("Moderate Stress" if anomaly > 1.2 else "Normal")
    return {
        "latitude": lat,
        "longitude": lon,
...
def get_carbon_intensity(country_code: str = Query("PK")):
    ...
    grid_data = {
        "PK": {"carbon_intensity": 380, "renewable_pct": 28.5, "fossil_pct": 71.5, "grid_status": "High Carbon"},
        "US": {"carbon_intensity": 360, "renewable_pct": 22.0, "fossil_pct": 78.0, "grid_status": "Moderate Carbon"},
        "UK": {"carbon_intensity": 145, "renewable_pct": 54.2, "fossil_pct": 45.8, "grid_status": "Low Carbon"},
        "JP": {"carbon_intensity": 430, "renewable_pct": 19.8, "fossil_pct": 80.2, "grid_status": "High Carbon"},
        "BR": {"carbon_intensity": 85, "renewable_pct": 83.0, "fossil_pct": 17.0, "grid_status": "Very Low Carbon"}
    }
    data = grid_data.get(country_code.upper(), {
...
def get_turbulence_risk(altitude_ft: int = Query(30000)):
    ...
    risk_level = "Severe" if cat_index > 7.0 else ("Moderate" if cat_index > 3.5 else "Light")
    return {
        "altitude_ft": altitude_ft,
        "cat_index": cat_index,
        "turbulence_risk": risk_level,
        "recommended_action": "Avoid airspace or adjust flight level" if cat_index > 7.0 else "Normal operation"
    }
...
def get_pollen_forecast(lat: float = Query(24.8607), lon: float = Query(67.0011)):
    ...
    risk_map = {0: "None", 1: "Very Low", 2: "Low", 3: "Moderate", 4: "High", 5: "Very High"}
    return {
        "latitude": lat,
        "longitude": lon,
        "pollen_counts": {"tree_pollen": tree, "grass_pollen": grass, "weed_pollen": weed},
...
def get_urban_heat_island(city: str = Query("karachi")):
    ...
    city_key = city.lower()
    city_info = CITIES_DB.get(city_key, {"name": city.capitalize(), "lat": 25.0, "lon": 67.0})
    ...
    return {
        ...
        "severity": "Critical UHI" if uhi_intensity > 4.0 else "Moderate UHI"
    }
...
def get_sovereign_risk(country: str = Query("Pakistan")):
    ...
    # Score 0-100 (Higher means more resilient)
    ratings = {
        "pakistan": {"score": 38.5, "rating": "High Vulnerability", "rank": 142},
        "united kingdom": {"score": 78.2, "rating": "High Resilience", "rank": 14},
        "united states": {"score": 74.0, "rating": "Moderate Resilience", "rank": 22},
        "japan": {"score": 81.5, "rating": "Very High Resilience", "rank": 8},
        "egypt": {"score": 45.0, "rating": "Moderate Vulnerability", "rank": 110}
    }
    data = ratings.get(country.lower(), {"score": 55.0, "rating": "Moderate Risk", "rank": 85})
...
def get_extreme_alerts():
    ...
    alerts = [
        {"id": "alt_101", "city": "Karachi", "event": "Heatwave Warning", "severity": "High", "lead_time_hrs": 24, "desc": "Temperatures expected to cross 41°C with high humidity."},
        {"id": "alt_102", "city": "London", "event": "Heavy Rain Risk", "severity": "Medium", "lead_time_hrs": 12, "desc": "Potential localized flash flooding in low-lying urban sectors."},
        {"id": "alt_103", "city": "Tokyo", "event": "Typhoon Proximity Alert", "severity": "High", "lead_time_hrs": 48, "desc": "High gale winds and heavy coastal precipitation approaching."},
        {"id": "alt_104", "city": "Cairo", "event": "Dust Storm Advisory", "severity": "Medium", "lead_time_hrs": 6, "desc": "Reduced visibility due to desert winds."}
    ]
...
def predict_microclimate(payload: MicroclimateInput):
    ...
    return {
        ...
        "model_used": "XGBoost/RandomForest Regressor" if HAS_SKLEARN else "Heuristic ML Fallback",
        "timestamp": datetime.utcnow().isoformat()
    }
...
def simulate_scenario(input_data: ScenarioInput):
    ...
    # Base baseline calculations
    base_temp_anomaly = 2.5
    base_carbon = 350.0

    # Adjustments based on interactive sliders
    ...
    simulated_temp_anomaly = max(-1.0, round(base_temp_anomaly + density_effect + veg_effect, 2))
    ...
    return {
        ...
        "simulated_outcomes": {
            ...
            "sustainability_score": min(100, max(0, int(100 - (simulated_temp_anomaly * 15) - (simulated_carbon * 0.1))))
        }
    }
...
def compare_cities(payload: CityCompareRequest):
    ...
    for city_name in payload.cities:
        ...
        info = CITIES_DB.get(city_key, {"name": city_name.capitalize(), "lat": 20.0, "lon": 70.0, "country": "Global"})
        
        # Aggregate real or calculated features for side-by-side comparison matrix
    ...
    return {"cities_compared_count": len(comparison_data), "comparison_matrix": comparison_data}
...
def get_global_analytics():
    ...
    return {
        ...
        "charts": {
            "temp_by_region": {"Asia": 28.4, "Europe": 19.2, "Americas": 22.1, "Africa": 31.0, "Oceania": 21.5},
            "energy_sources": {"Fossil": 58, "Hydro": 18, "Wind/Solar": 16, "Nuclear": 8}
        }
    }
...
def ai_chatbot(chat: ChatMessage):
    ...
    return {"user_query": chat.message, "ai_response": reply, "timestamp": datetime.utcnow().isoformat()}


@app.post("/api/v1/subscribe", tags=["Interactive Tools"])
def subscribe_alerts(sub: SubscriptionRequest, background_tasks: BackgroundTasks):
    """Feature 20 - Email Alert Subscription Service."""
    sub_entry = {"email": sub.email, "country": sub.country, "subscribed_at": datetime.utcnow().isoformat()}
    SUBSCRIBERS_DB.append(sub_entry)
    logger.info(f"New user subscribed: {sub.email} for country {sub.country}")
...
async def websocket_alerts_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            # Simulate real-time monitoring push loop every 15 seconds
            ...
            push_payload = {
                ...
                "new_alert": random.choice([
                    None,
                    {"city": "Sydney", "type": "High Wind Warning", "severity": "Medium"},
                    {"city": "Cairo", "type": "Heat Index Elevation", "severity": "High"}
                ])
            }
...
# =====================================================================
# 10. ROOT ROUTE & STATIC FILE SERVING
# =====================================================================
...
def root():
    return {
        "platform": "EcoPulse Climate Intelligence",
        "status": "Operational",
        "documentation": "/docs",
        "dashboard": "/dashboard"
    }
